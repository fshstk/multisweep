  ...
  private:
    void paint(Graphics& g) override
    {
      ...
      for (float dB = -10.0f; dB >= minLevel; dB -= 5.0f)
        lastTextDrawPos = drawLevelMark(
          g, xPos, width, dB, String(roundToInt(-dB)), lastTextDrawPos);
    }
    ...
    const int inline drawLevelMark(Graphics& g,
                                   int x,
                                   int width,
                                   const int level,
                                   const String& label,
                                   int lastTextDrawPos = -1)
    {
      ...
      if (yPos - 4 > lastTextDrawPos) {
        g.drawText(
          label, x + 2, yPos - 4, width - 4, 9, Justification::centred, false);
        return yPos + 5;
      }
...
public:
  ...
  void paint(Graphics& g) override
  {
    ...
    if (isGRmeter)
      lvlRect = Rectangle<int>(
        Point<int>(meterArea.getX(), overlay.getOffset()),
        Point<int>(meterArea.getRight(), overlay.decibelsToY(level)));
    else
      lvlRect = Rectangle<int>(
        Point<int>(meterArea.getX(), height),
        Point<int>(meterArea.getRight(), overlay.decibelsToY(level)));
