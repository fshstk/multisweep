...
public:
  ...
  /** Attempts to set the current position of the stream. Returns true if this
   * was successful. */
  ...
  OSCAddress readAddress() { return OSCAddress(readString()); }
  ...
  OSCTypeList readTypeTagString()
  {
    ...
    checkBytesAvailable(
      4, "OSC input stream exhausted while reading type tag string");
  ...
  OSCArgument readArgument(OSCType type)
  {
    switch (type) {
      ...
      default:
        // You supplied an invalid OSCType when calling readArgument! This
        // should never happen.
        jassertfalse;
        throw OSCInternalError(
  ...
  OSCBundle readBundle(
    size_t maxBytesToRead = std::numeric_limits<size_t>::max())
  {
    ...
    if (readString() != "#bundle")
      throw OSCFormatError("OSC input stream format error: bundle does not "
                           "start with string '#bundle'");
  ...
  {
    checkBytesAvailable(
      4, "OSC input stream exhausted while reading bundle element size");
...
private:
  ...
  OSCBundle readBundleWithCheckedSize(size_t size)
  {
    ...
    auto maxBytesToRead =
      size - 4; // we've already read 4 bytes (the bundle size)
    ...
    if (getPosition() - begin != size)
      throw OSCFormatError("OSC input stream format error: wrong element "
                           "content size encountered while reading");
