...
public:
  ...
  AudioProcessorBase(const BusesProperties& ioLayouts,
                                                  ...
                                                  float opt) override
  {
    ...
    if (index == 0x0069656D) // prefix 00 chooses OSC message
    {
      try {
        size_t size =
          static_cast<size_t>(value); // let's make this the data size
  ...
  OSCParameterInterface& getOSCParameterInterface()
  {
    return oscParameterInterface;
  }
